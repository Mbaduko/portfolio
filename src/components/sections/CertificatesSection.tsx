            ...
            <div className="flex space-x-2 min-w-full">
              {categories.map((category) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-t-xl border-b-2 font-semibold text-sm transition-all duration-300 whitespace-nowrap
                    ${activeFilter === category.id
                      ? 'border-primary-button text-primary-button bg-background'
                      : 'border-transparent text-foreground bg-secondary-bg/60 hover:bg-secondary-bg/80'}
                  `}
                >
                  {category.name}
                  <span className="ml-2 text-xs text-accent-text">{category.count}</span>
                </button>
              ))}
            </div>
          </div>
