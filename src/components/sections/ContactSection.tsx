  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ fullName: formData.name, email: formData.email, message: formData.message }),
      });
      ...
      if (res.ok && data.status === 'success') {
        ...
        // auto-clear success after short delay
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <button
                  ...
                  className={`w-full py-4 px-6 rounded-xl font-semibold transition-all duration-300 transform shadow-lg ${
                    status === 'sending'
                      ? 'bg-primary-button/70 text-white cursor-wait'
                      : 'bg-primary-button text-white hover:bg-primary-button/90 hover:scale-105'
                  }`}
                >
                ...
                {status === 'success' && (
                  <p className="mt-3 text-sm text-primary-button">Message sent — thank you!</p>
                )}
                {status === 'error' && (
                  <p className="mt-3 text-sm text-red-500">Failed to send message. Please try again later.</p>
                )}
              </div>
            </form>
          </div>
